...
def make_check_kwargs(job_id: str, number: str):
    # Retrieve BigQuery destination table within task execution context to avoid top-level overhead
...
with DAG(
    ...
    # Optimized: Refactored from static 1000-task loop to Dynamic Task Mapping (expand)
    # This prevents the scheduler from having to parse 4,000 static operator instances
    # on every heartbeat, reducing parse time to milliseconds.
