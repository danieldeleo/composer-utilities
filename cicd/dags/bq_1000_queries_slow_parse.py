...
# Optimization Notes:
# Refactored from static loop generation (4,000 task instances) to Dynamic Task Mapping (.expand).
# This prevents DAG parsing timeouts by shifting per-item expansion from DAG parsing time to execution time.
# BigQueryHook execution is encapsulated within the task callable to avoid top-level database hits (Best Practice 1).
# Standard default_args configured with retries and retry_delay (Best Practice 6).
